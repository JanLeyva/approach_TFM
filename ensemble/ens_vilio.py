...
def main(path, gt_path="./results/"):
    ...
    """
    # Ground truth
    dev_df = pd.read_json(os.path.join(gt_path, 'test_unseen.jsonl'), lines=True)
    ...
    while (delta > 0.0001):
        ...
        for i, column in enumerate(test_unseen_probas):
            score = roc_auc_score(dev_df.label, test_unseen_probas.iloc[:, i])
            print(column, score)
        ...
        print(roc_auc_score(dev_df.label, test_unseen_SA.proba), accuracy_score(dev_df.label, test_unseen_SA.label))
        print('-'*50)
        ...
        print(roc_auc_score(dev_df.label, test_unseen_SX.proba), accuracy_score(test_unseen_SX.label, dev_SX.label))
        print('-'*50)
        ...
        delta = abs(roc_auc_score(dev_df.label, test_unseen_SX.proba) - last_score)
        last_score = roc_auc_score(dev_df.label, test_unseen_SX.proba)
    ...
    fpr, tpr, thresholds = metrics.roc_curve(dev_df.label, test_unseen_SX.proba)
